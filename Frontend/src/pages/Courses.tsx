...
import { BookOpen, Clock, User, CheckCircle, AlertCircle, Search, Users, Loader2 } from "lucide-react";
...
function CourseDetail({ course, onBack, onEnroll }: { course: Course; onBack: () => void; onEnroll: (course: Course) => void }) {
      ...
      <div className="glass-surface p-6" style={{ boxShadow: "var(--shadow-card)" }}>
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">{course.credits} créditos</p>
          </div>
          <Button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="glass-surface p-5" style={{ boxShadow: "var(--shadow-card)" }}>
            ...
            <p className="text-sm text-muted-foreground">No disponibles en la versión actual</p>
          </div>
        </div>
          ...
          <div className="glass-surface p-5 space-y-4" style={{ boxShadow: "var(--shadow-card)" }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">{course.credits} créditos</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">Semestre {course.semester}</p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-surface p-5" style={{ boxShadow: "var(--shadow-card)" }}>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-foreground font-medium">{course.occupied_slots} / {course.slots}</span>
              </div>
...
export default function Courses() {
  ...
  const [selectedForEnroll, setSelectedForEnroll] = useState<Course | null>(null);
  ...
  const semesters = Array.from(new Set(courses.map((c) => c.semester || 1))).sort((a, b) => a - b);

  const filtered = courses.filter((c) => {
    const matchSearch = c.name.toLowerCase().includes(search.toLowerCase()) || c.code.toLowerCase().includes(search.toLowerCase());
    const matchSemester = semesterFilter === "all" || c.semester === parseInt(semesterFilter);
  ...
  const handleConfirmEnroll = async () => {
    ...
    if (selectedForEnroll) {
      toast({ title: "¡Inscripción exitosa!", description: `Te has inscrito en ${selectedForEnroll.name}` });
    }
    setEnrolling(false);
    ...
    return (
      <AppLayout>
        <div className="flex-1 overflow-y-auto scrollbar-thin px-6 py-6">
          <CourseDetail course={selected} onBack={() => setSelected(null)} onEnroll={handleEnroll} />
        </div>
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        <div className="px-6 pt-6 pb-4">
          ...
          <p className="text-sm text-muted-foreground mt-1">Explora y inscríbete en tus cursos electivos</p>
        </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Select value={availabilityFilter} onValueChange={setAvailabilityFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="unavailable">Sin cupos / Cerrados</SelectItem>
              </SelectContent>
            </Select>
          </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <Badge variant="secondary" className="bg-secondary text-secondary-foreground text-[10px]">Sem. {course.semester}</Badge>
                    </div>
                    <h3 className="font-display font-semibold text-foreground group-hover:text-primary transition-colors mb-1">
                      {course.name}
                    </h3>
                    <p className="text-xs text-muted-foreground mb-3">{course.credits} créditos</p>
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-xs">
                        <span className="text-muted-foreground">{course.occupied_slots}/{course.slots} inscritos</span>
        ...
        <DialogContent className="bg-card border-border">
          ...
          {selectedForEnroll && (
            <div className="p-4 rounded-lg bg-secondary/50 space-y-3">
              <div>
                <p className="text-sm font-semibold text-foreground">{selectedForEnroll.name}</p>
                <p className="text-xs text-muted-foreground mt-1">{selectedForEnroll.code} · Sem. {selectedForEnroll.semester}</p>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>{selectedForEnroll.slots - selectedForEnroll.occupied_slots} cupos disponibles</span>
              </div>
            </div>
          )}
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleConfirmEnroll} disabled={enrolling} className="bg-primary text-primary-foreground hover:bg-primary/90">
              {enrolling ? <Spinner size="sm" className="border-primary-foreground/30 border-t-primary-foreground" /> : "Confirmar inscripción"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
